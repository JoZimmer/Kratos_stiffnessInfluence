...
class ImposeMeshMotionProcess(KratosMultiphysics.Process):
    ...
    def __init__(self, Model, params):
        ...
        x0y0 = params['x0y0'].GetVector()
        if x0y0.Size() != 2:
            raise Exception(
                'The reference point position x0y0 has to be provided with 2 coordinates!')
        self.x0y0 = x0y0

        dxdy = params['dxdy'].GetVector()
        if dxdy.Size() != 2:
            raise Exception(
                'The dxdy relative motion has to be provided with 2 coordinates!')
        self.dxdy = dxdy

        # user inpput expected in degrees, here changing to radians
        self.theta = math.radians(params['dtheta'].GetDouble())
    ...
    def ExecuteInitializeSolutionStep(self):
        for node in self.model_part.Nodes:
            ...
            node.SetSolutionStepValue(
                KratosMultiphysics.DISPLACEMENT_X, dx + self.dxdy[0])
            node.SetSolutionStepValue(
                KratosMultiphysics.DISPLACEMENT_Y, dy + self.dxdy[1])
